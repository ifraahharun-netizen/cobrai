...
function parseCurrencyMinor(
    value: unknown,
) {
    const pounds = parseNumber(
        value,
        Number.NaN,
    );
...
export function parseRetentionCsv(
    ...
    if (parsed.errors.length > 0) {
        ...
        throw new Error(
            `CSV could not be parsed: ${message}`,
        );
    }
    ...
    const mappedValues = [
        ...mappedHeaders.values(),
    ];

    const hasCustomerName =
        mappedValues.includes(
            "customerName",
        );
    ...
    const rowsWithProvidedScores =
        rows.filter(
            (row) =>
                row.providedRiskScore !==
                null,
        ).length;

    if (
        rowsWithProvidedScores > 0 &&
        rowsWithProvidedScores <
        rows.length
    ) {
        warnings.push(
