...
export async function POST(req: Request) {
    try {
        ...
        if (workspace.tier !== "pro") {
            return jsonError(
                "Send notification is available on Pro.",
                403,
        ...
        const title = normalizeText((raw as any).title);
        const message = normalizeText((raw as any).message);
        ...
        if (!title || !message) {
            return jsonError("Missing title or message", 400);
        }
        ...
        if (!linkedCustomerId) {
            return jsonError("No linked customer found for notification", 400);
        }
        ...
        await prisma.actionExecution.create({
            data: {
                ...
                channel: "manual",
                title: companyName ? `${companyName} notification` : title,
                subject: title,
                body: message,
                status: "pending",
                metadata: {
        ...
        return NextResponse.json({
            ok: true,
            message: "Notification action created.",
        });
    } catch (e: any) {
        ...
        return NextResponse.json(
            { ok: false, error: "Failed to process notification action" },
            { status: 500 }
        );
    }
}